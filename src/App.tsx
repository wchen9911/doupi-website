...
function AdminPanel({ stats, history, onUpdateStats, onUpdateHistory, onExit }: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 sky-bg pixel-font text-black">
...
function QuestJournal({ history, onExit }: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 sky-bg pixel-font text-black">
      <div className="bg-white border-8 border-black p-8 w-full max-w-4xl max-h-[90vh] overflow-y-auto text-black text-center">
        <div className="flex justify-between items-center mb-8"><h2 className="text-xl uppercase font-bold text-black text-center">Quest Journal</h2><button onClick={onExit} className="text-red-600 uppercase text-xs font-bold text-black">[ Close ]</button></div>
...
function MorningPowerUp({ onComplete }: { onComplete: (items: string[]) => void }) {
  ...
  return (
    <div className="max-w-2xl mx-auto my-12 bg-white border-8 border-black p-8 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] pixel-font relative text-black text-left">
      ...
      <h2 className="text-xl mb-8 uppercase flex items-center gap-4 text-black font-bold text-center"><Heart className="fill-red-600 text-red-600 animate-pulse" /> Morning Buffs</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 text-black">
        {tasks.map(t => (
          <button key={t.id} onClick={() => setTasks(tasks.map(task => task.id === t.id ? { ...task, done: !task.done } : task))} className={`flex items-center gap-4 p-4 border-4 border-black transition-all ${t.done ? 'bg-green-100' : 'bg-white hover:bg-gray-50'} text-black`}><div className={`w-10 h-10 border-4 border-black flex items-center justify-center shrink-0 ${t.done ? 'bg-green-500' : 'bg-white'} text-black`}><t.icon size={20} /></div><span className={`text-[10px] text-left leading-tight uppercase ${t.done ? 'line-through text-gray-400' : 'text-black font-bold'}`}>{t.text}</span></button>
        ))}
      </div>
...
function BattleUI({ gameId, problem, feedback, progress, totalProgress, timeLeft, onExit, onAnswer }: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col sky-bg pixel-font overflow-hidden text-white">
      ...
      <button onClick={onExit} className="fixed bottom-4 right-4 p-4 bg-green-500 border-4 border-black text-white z-30 uppercase font-bold shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-center"><RotateCcw size={24} /></button>
    </div>
  );
}
...
function FocusTimer({ title, targetMins, onExit, onComplete, onLogTime }: any) {
  ...
  return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 sky-bg pixel-font text-black text-center"><div className="bg-white border-8 border-black p-10 max-w-md text-black text-center"><h2 className="text-xl mb-4 uppercase text-black font-bold text-center">{title}</h2><div className="text-6xl mb-8 text-black font-bold text-center">{formatTime(seconds)}</div><button onClick={() => setIsActive(!isActive)} className={`w-full text-white p-6 border-4 border-black mb-4 font-bold shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${isActive ? 'bg-red-500' : 'bg-green-500'} text-center uppercase`}>{isActive ? 'PAUSE' : 'START'}</button><button onClick={() => { onLogTime((Date.now() - startRef.current) / 60000); onExit(); }} className="text-[8px] text-gray-400 uppercase w-full font-bold text-center underline">Exit Mission</button></div></div>);
}
  ...
  return (
    <div className="min-h-screen sky-bg pixel-font pb-20 selection:bg-red-600 selection:text-white relative flex flex-col text-black">
      ...
      <header className="pt-16 pb-8 px-6 text-center z-10 shrink-0 text-black">
        <div className="relative inline-block"><div className="absolute -inset-4 bg-black border-4 border-white translate-x-2 translate-y-2"></div><div className="relative bg-[#e76d42] border-4 border-black p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] text-white font-bold"><h1 className="text-2xl md:text-4xl leading-tight uppercase tracking-tighter font-bold text-center">SUPER <br /> OWEN WORLD</h1></div></div>
      </header>
